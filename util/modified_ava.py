...
from util.labeled_video_dataset import LabeledVideoDataset
...
class AvaLabeledVideoFramePaths:
    ...
    def load_and_parse_labels_csv(
        ...
        """
        Parses the frame labels .csv file.
        Args:
            frame_labels_file (str): Path to the file containing labels
                per frame. The expected format is:
                <original_vido_id, frame_index, bbox_x_1, bbox_y_1, ...
        ...
        Returns:
            (dict): A dictionary of dictionary containing labels per each frame
                in each video. Here, the label for each frame is again a dict
                of the form,
                {
                    'labels': a list of bounding boxes
                    'boxes': a list of action labels for the bounding box
                    'extra_info': list of extra information containing either
        ...
        with g_pathmgr.open(frame_labels_file, "r") as f:
            for line in f:
                ...
                if frame_index not in labels_dict[video_idx]:
                    labels_dict[video_idx][frame_index] = defaultdict(list)

                labels_dict[video_idx][frame_index]["boxes"].append(bbox)
                labels_dict[video_idx][frame_index]["labels"].append(label)
                labels_dict[video_idx][frame_index]["extra_info"].append(extra_info)
        return labels_dict
...
class TimeStampClipSampler(ClipSampler):
    """
    A specialized clip sampler for sampling video clips around specific timestamps.
    This is particularly used in datasets like Ava wherein only a specific subset
    of clips in the video have annotations.
    """

    def __init__(self, clip_sampler: ClipSampler, fps: float) -> None:
        """
        Args:
...
def Ava(
    ...
    return LabeledVideoDataset(
        ...
        clip_sampler=TimeStampClipSampler(clip_sampler, fps=30),
        transform=transform,
