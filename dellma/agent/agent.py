...
- explanation: a string that describes, in detail, the reasoning behind your decision. """
            ...
            format_instruction += f"""You should include information on the expected actions and discussion of each grade, as well as factors that affect them."""
            return format_query(
...
For example, if one of the state variable is 'actions completeness', and the top 3 most likely values are 'thorough', 'adequate', and 'lacking', then your response should be formatted as follows:
{{
...
"""
    ...
    {{
        ...
        Each key should map to a JSON object with 3 keys, one is 'reward', which is a float that describes the reward of the action, and the other is 'unit', which is a string that describes the unit of the reward. Then you should put your reasoning of this action under key 'explanation'. For example, if the reward of action 'set to grade good' is list of actions, then your response should be formatted as follows:
    {{
        ""action 1: grade good"": {{
            ""reward"": list of actions,
            ""explanation"": ""I think the set of actions performed per the discussions had is good, so I will grade it good.""
        }},
        ...
    }}"""
            decision_prompt_func = (
